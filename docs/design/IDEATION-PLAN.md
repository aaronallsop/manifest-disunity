...
| **5** ◀ **LIVE** | **Diplomacy** | Reaches the scoreboard only through the others, so it goes late and inherits. It is also the round where a *new* nation's problems land — recognition, alliances against a conqueror, being nobody. **It opens shorter than the others**: round 3's thirteen federation rulings on 11 September already settled what an alliance is, what a bloc costs, how you join one and how you leave. **One item is blocking** — the diplomatic act that speeds a thaw, without which the 33 pairs that open Hostile have no move at all |
