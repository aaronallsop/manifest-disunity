...
| ~~**5**~~ | ~~**Does the AI read its tree?**~~ **✅ ANSWERED FOR THE ALPHA — D236, 15 September 2026.** Aaron: *"Lets have it be something for the alpha where **the mission tree only applies to the player if they pick that nation.** The logic is **testing states with the same mission tree but different starting paths**."* **So for the alpha there is no AI tree to read, and the answer to "is the player racing an opponent that does not know the race is on" is that there is no opponent in the race at all.** ⚠ *It also DISSOLVES the race-versus-checklist card rather than answering it — if only the player has a tree, two nations can never meet the same condition.* **⚠ What it costs: the three shared trees were designed BECAUSE the nations are rivals for one prize, and under this ruling that rivalry is not simulated.** *The right trade for an alpha testing one tree from several starts — and **whether AI nations get trees afterwards stays open, and stays his and the build order's*** | ✅ **Aaron, for the alpha** |
...
| **7** | **Nothing says what a mission's reward does when the ground that earned it is lost.** §1.1 rules the *mission* permanent; whether a modifier tied to holding a place survives losing the place is a different question and is unasked *⚠ 24 Sep 2026 (D267): Aaron approved the card on this on the Control Board, 15 September, with no note — the recommendation as written.* |
