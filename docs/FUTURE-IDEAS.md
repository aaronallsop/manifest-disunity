...
## F24 — A wiki for the design — ✅ **BUILT, 12 September 2026**
...
**Built the next day.** `docs/wiki/` — 81 pages over eleven master topics, generated by
...
**What it turned out to be, against what this entry imagined.** The entry pictured a page per thing
with a ruling number beside every claim. Aaron's own brief on the day replaced that with an
encyclopaedia modelled on the Europa Universalis 4 wiki: topic pages that read as articles, three
sections each — *what it is*, *how it works in the game*, *the story behind it* — with the links
between pages as a deliverable in their own right, because the graph view is how he wants to see the
shape of the game. **The prose is written by hand, by a hired writer, and that is not a breach of the
"generated, not hand-written" rule** — the rule protects facts, and no fact is restated in these
pages. Figures are read from source on every run inside blocks the generator owns; prose lives
outside them and is never overwritten. Proved by test: a page edited by hand survives regeneration
intact.

**What this entry got wrong, corrected by measurement.** It said *"147 rulings across three rounds"*.
Counted 12 September: **135 numbered rulings** — 53, 41 and 41 — or 136 counting politics' half-ruling
11a. And it said Aaron's Sagebrush question had an answer living in four places; it lives in **nine**,
in three mutually incompatible schemes, of which the four that the running game actually uses all say
*Republican*.

**Its stated prerequisite is half met.** *"A stable id per movement and per nation."* Movements: done —
all 32 ids are clean, and the join is recorded in `build/wiki_aliases.json`. **Nations: still missing.**
There is no committed file anywhere holding a nation id; the only nation-shaped file is an autosave
from turn 4 of one playthrough, and it is not backed up. Nation pages are keyed on display names until
somebody authors a roster.

**What it fixed on the way.** Every movement's **verb** and **adjective** — the attributes rounds 1–3
rule on more than any other — existed only on a published web page outside the repository. They are now
in `build/wiki_movements.json` with Aaron's own notes, and every live movement's page states its verb.

**What is still owed.** Economy has no pages — its round is open. Events has none and is two rounds
away, though twelve authored crises already exist in the content folder. People has no page of its own.
