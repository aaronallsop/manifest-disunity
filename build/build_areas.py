...
def main():
    ...
    while changed:
        ...
        # smallest deficient eastern group first
        for r in sorted(roots, key=lambda r: gpop[r]):
            ...
            if not neigh_roots:
                continue
            target = min(neigh_roots, key=lambda x: gpop[x])
            union(r, target)
            changed = True
    ...
    out = {"threshold": THRESHOLD, "areas": areas}
    dest = os.path.join(DATA, "areas.json")
